...
def test_low_completion_ratio_triggers_spillover_even_without_explicit_carryover():
    # completed=7/10=0.7 < 0.8 threshold, not_completed_count reported as 0
    # (e.g. issues removed/punted rather than carried over).
    df = pd.DataFrame(
        [_sprint_row(completed_issue_count=7, not_completed_count=0)]
    )
    labeled = add_labels(df)
    assert labeled.loc[0, "label_spillover"] == 1
